---
title: Docker & Podman
description: Run AI Monitor Pro in a container with Docker or Podman using the bundled Makefile and Compose files.
---

import { Callout } from 'fumadocs-ui/components/callout';

Instead of the native launcher, you can run AI Monitor Pro in containers. The repo ships a `Makefile` and Compose files that work with either **Docker** or **Podman** — `make` auto-detects which one you have, preferring `podman compose` when both are present.
...
The `:ro` flag keeps each mount read-only, so AI Monitor Pro can never modify your agent logs. The trailing `:z` relabels the mount for SELinux-enforcing hosts (Fedora, RHEL, CentOS); it is harmless on macOS and other Linux distros.
...
AI Monitor Pro's own state — `history.db`, cached summaries, budgets, settings — lives on a named Docker/Podman volume called `tt_data`, mounted at `/tt-data` inside the container (via `TOKENTELEMETRY_DATA_DIR` and `TOKENTELEMETRY_HOME`).
...
This pulls `ghcr.io/vasihemanth/ai-monitor-pro-backend` and `ghcr.io/vasihemanth/ai-monitor-pro-frontend`. On a fork, set `GHCR_OWNER` to your own namespace. `TT_IMAGE_TAG` selects the tag (default `latest`).
