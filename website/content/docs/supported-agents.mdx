...
description: The 10 coding agents and Hermes autonomous agent that AI Monitor Pro tracks, how each is detected, and what data each captures.
---

AI Monitor Pro reads log files from the agents listed below. Detection is automatic — no configuration needed. Just run your agent as normal and AI Monitor Pro picks up the sessions.
...
Antigravity is Google's browser automation agent. AI Monitor Pro reads its sessions from `~/.gemini/antigravity/` and additionally captures browser screenshots and Antigravity recording strips (thumbnail sequences from `browser_recordings`). See [Artifacts](/docs/features/artifacts).

### Cursor

Cursor stores its chat sessions in `~/.cursor/` and VS Code's `workspaceStorage/`. AI Monitor Pro merges sessions from both locations. Plans extracted from Cursor's `/plan` output are available in the Projects → Plans tab.
...
Hermes writes its state to `~/.hermes/` (or the directory pointed to by `$HERMES_HOME`). AI Monitor Pro reads:
...
Don't see your agent? [Open a GitHub issue](https://github.com/richar12138/ai-monitor-pro/issues) with the agent name and its log file format and we'll add support.
