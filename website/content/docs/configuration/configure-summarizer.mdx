...
Open Settings → Summarizer in the AI Monitor Pro UI to configure your backend.
...
All backends are invoked locally. For CLI-based backends (Claude, Codex, Gemini, Qwen, Antigravity), AI Monitor Pro shells out to the CLI binary you already have installed. The OpenAI-compatible backend uses stdlib `urllib` — no new Python dependencies.
...
The selected model is stored in `~/.ai-monitor-pro/summarizer.json` and applied to all future summarization requests.
...
<Callout type="info">
Non-OpenAI parameters (top_k, min_p, repetition_penalty, enable_thinking) are sent first. If the server rejects them with HTTP 400, AI Monitor Pro automatically retries with a clean OpenAI-only payload so the summary still goes through. You don't need to toggle this manually.
</Callout>
...
Set them before launching AI Monitor Pro:

```bash
TT_OLLAMA_TIMEOUT=600 ai-monitor-pro
```
...
Summarizer configuration is stored in `~/.ai-monitor-pro/summarizer.json`. You can edit it directly if needed — it's plain JSON. The file is created on first save.
