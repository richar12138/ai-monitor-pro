...
<Callout title="Observational only">A budget never blocks or throttles an agent. TokenTelemetry reads logs after a session ends, so it can report that you went over — it can't stop a run mid-flight.</Callout>
