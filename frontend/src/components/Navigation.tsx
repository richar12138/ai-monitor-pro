...
export default function Navigation({ isCollapsed, setIsCollapsed }: NavigationProps) {
  ...
  const LINKS = [
    ...
    { name: t("nav.localModels"), href: "/local-models", icon: Zap },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5">
        {[
          ...
          ...(availableAgents.includes("hermes")
            ? [{ name: t("hermes.agent"), href: "/hermes", icon: HermesIcon as typeof LayoutDashboard }]
            : []),
        ].map((link) => (
      ...
      <div className="mt-auto space-y-3">
        ...
        <NavLink
          link={{ name: t("nav.settings"), href: "/settings", icon: Settings2 }}
          pathname={pathname}
        ...
        >
          {isCollapsed
            ...
            : (<><PanelLeftClose size={14} /><span className="text-[10px] uppercase tracking-[0.18em]">{t("nav.collapse")}</span></>)}
        </button>
      </div>
    </nav>
  );
